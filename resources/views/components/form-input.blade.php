...
>
    ...
    <input
        ...
        :class="($store.form.submitted && missing) @if($type === 'email') || invalidEmail @endif ? 'border-rose-400 focus:border-rose-500 focus:ring-rose-500/10' : 'border-slate-300 focus:border-amber-500 focus:ring-amber-500/10'"
        class="w-full rounded-lg border px-4 py-3 outline-none transition focus:ring-4"
    >
    <p x-show="$store.form.submitted && missing" x-cloak class="mt-2 text-sm text-rose-600">This field is required.</p>
    @if($type === 'email')
